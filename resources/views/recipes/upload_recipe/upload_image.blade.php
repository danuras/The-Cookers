...
<body>
    <div id="page1" class="page active">
        ...
        @if (session('image_url_r'))
            <img class="shadow" id="preview" src="{{ asset(session('image_url_r')) }}" alt="Profile">
        @else
    ...
    <center>
        ...
        <!-- Tombol Selanjutnya -->
        <form action="{{ route('save.recipes.upload-image') }}" method="POST"
            style="display: inline-block; vertical-align: middle;" enctype="multipart/form-data">
            @method('POST')
            @csrf
            <input type="file" accept="image/jpeg, image/png, image/jpg" id="upload-input" name='image_url'
                class="form-control" onchange="previewImage(event)" hidden />
